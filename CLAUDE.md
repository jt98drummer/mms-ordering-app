...
8. **Refinements:** tighten color maps (e.g., "Navy" resolves to Bella "Heather Midnight Navy"); **the Structured Cap (`ap10`, Printful product 422 = Richardson 112) has NO Navy/Red variant** — real colors are Black / Charcoal / Heather Grey / Loden, so ordering a Navy or Red cap would fail `resolve_variant()` at fulfillment and the mockup fell back to Black; fix `ap10.colors` + `color_map` in `swag_catalog.json` to real colors. Set proper embroidery placements (polo left-chest, cap front, beanie cuff) + per-garment thread palette; scope app-only `Mail.Send` with an Exchange Application Access Policy.
...
- `gen_products.py` — generates `assets/products/<id>.png` (PIL silhouettes for promo/vendor items + Printful mockups for apparel, flattened onto a shared studio backdrop); `printful_mockups.py` — Printful Mockup Generator client (printfiles → placement/position → create-task → poll → download; 429 backoff).
- `templates/`, `static/`, `assets/` (`assets/print/` = logos, `assets/products/` = storefront images, `assets/tiles/` = homepage banners), `.github/workflows/keepalive.yml`.
