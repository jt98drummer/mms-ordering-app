...
- `branding.py` — the 4 MMS logo variants, garment-colour → tone → logo options, print-file URLs, embroidery thread palettes, and colour chips (single source of truth for the logo picker + fulfillment).
- `budget.py` — per-user swag budget: bimonthly calendar periods, per-role budget amounts, spend store (`spent()`/`add_spend()`/`status()`), reset date. Backing store is a JSON file (swap for a durable store on Render).
